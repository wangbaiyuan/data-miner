...
const getAllGHIssueComments = async () => {
  const comments = await client
    .request(queryAllGHIssueComments, { name, owner })
    .then((data: any) =>
      data.repository.issues.nodes
        .filter((issue: any) => (issue.comments.totalCount > 0))
        .map((issue: any) => {
          const comment_post_hashID = issue.labels.nodes.filter(
            (label: any) => label.name !== "Gitalk"
          )[0].name;
          return issue.comments.nodes.map((comment: any) => ({
